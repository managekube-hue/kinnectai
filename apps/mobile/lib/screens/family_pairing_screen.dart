...
class _FamilyPairingScreenState extends State<FamilyPairingScreen> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      floatingActionButton: FloatingActionButton.extended(
        ...
        icon: const Icon(Icons.person_add, color: KinnectColors.darkBg),
        label: const Text('Add Teen', style: TextStyle(color: KinnectColors.darkBg)),
      ),
    );
  }
  ...
  Widget _buildEmptyState() {
    return Center(
      child: Column(
        ...
        children: [
          const Icon(Icons.family_restroom, size: 80, color: KinnectColors.grey40),
                ...
                Row(
                  children: [
                    CircleAvatar(
                      ...
                      child: Text(
                        account.name[0],
                        style: const TextStyle(color: KinnectColors.amber, fontSize: 20, fontWeight: FontWeight.bold),
                      ),
                    ),
                    ...
                    IconButton(
                      icon: const Icon(Icons.settings, color: KinnectColors.amber),
    ...
    return Column(
      ...
      children: [
        Row(
          ...
          children: [
            const Text('Today\'s Usage', style: TextStyle(color: KinnectColors.grey60, fontSize: 12)),
            Text(
        ...
        ClipRRect(
          ...
          child: LinearProgressIndicator(
            value: percentage,
            minHeight: 8,
            backgroundColor: KinnectColors.grey20,
  ...
  Widget _buildControlChip(String label, IconData icon, VoidCallback onTap) {
    return InkWell(
      onTap: onTap,
      child: Chip(
        avatar: Icon(icon, color: KinnectColors.amber, size: 16),
        label: Text(label, style: const TextStyle(color: KinnectColors.white, fontSize: 12)),
        backgroundColor: KinnectColors.darkBg,
      ),
    );
  }
  ...
  void _showAddTeenDialog() {
    showDialog(
      ...
      builder: (context) => AlertDialog(
        backgroundColor: KinnectColors.darkSurface,
        title: const Text('Add Teen Account', style: TextStyle(color: KinnectColors.white)),
        content: Column(
          ...
          children: [
            const TextField(
              ...
              decoration: InputDecoration(
                ...
                labelStyle: TextStyle(color: KinnectColors.grey60),
                enabledBorder: UnderlineInputBorder(borderSide: BorderSide(color: KinnectColors.grey40)),
              ),
            ),
            const SizedBox(height: 16),
            const TextField(
        ...
        actions: [
          ...
          ElevatedButton(
            ...
            child: const Text('Send Invite', style: TextStyle(color: KinnectColors.darkBg)),
          ),
        ],
      ),
    );
        ...
        child: Column(
          ...
          children: [
            Text(
              ...
              style: const TextStyle(color: KinnectColors.white, fontSize: 20, fontWeight: FontWeight.bold),
            ),
            const SizedBox(height: 24),
            ListTile(
              leading: const Icon(Icons.timer, color: KinnectColors.amber),
              title: const Text('Daily Time Limit', style: TextStyle(color: KinnectColors.white)),
              subtitle: Text('${account.dailyLimit} minutes', style: const TextStyle(color: KinnectColors.grey60)),
              onTap: () => _showScreenTimeDialog(account),
            ),
            ListTile(
              leading: const Icon(Icons.visibility, color: KinnectColors.amber),
              title: const Text('Account Type', style: TextStyle(color: KinnectColors.white)),
              subtitle: Text(account.accountType, style: const TextStyle(color: KinnectColors.grey60)),
              onTap: () {},
            ),
            ListTile(
              leading: const Icon(Icons.message, color: KinnectColors.amber),
              title: const Text('Message Restrictions', style: TextStyle(color: KinnectColors.white)),
              subtitle: const Text('Confirmed Kinnections only', style: TextStyle(color: KinnectColors.grey60)),
              onTap: () {},
            ),
            ListTile(
        ...
        content: Column(
          ...
          children: [
            ...
            const Text(
              ...
              style: TextStyle(color: KinnectColors.grey60, fontSize: 12),
            ),
          ],
        ),
        actions: [
